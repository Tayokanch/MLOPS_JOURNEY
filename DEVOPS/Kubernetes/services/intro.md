...
1. `NodePort Service`: This is where the service makes an internal port accesible on a port on the Node.
2. `ClusterIp Service`: In this case the service create a **Virtual IP** inside the cluster and enable communication between different services such as a set of **frontend** and **Backend** Servers
3. `LoadBalancer Service`: In the case Service provisions a Load Balancer for our application in supported cloud providers. E.g, To distribute loads across the different web servers in the Node


# A USE CASE OF K8s Service with a Single Pod
...
# A USE CASE OF K8s Service with a Multiple Pods
...
# A use case of K8s Service when same Pods are distributed across multiple nodes (Node Cluster) ?
